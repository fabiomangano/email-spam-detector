...
export function computeEmailMetrics(parsedData: any): EmailTechnicalMetrics {
  const email = parsedData.parsed || parsedData;
  const allText = (email.htmlText || '') + (email.plainText || '');

  console.log("emailText", email.htmlText)
  console.log("emailPalinText", email.plainText)

  const numLinks = (allText.match(/https?:\/\//g) || []).length;
  const numImages = ((email.htmlText || '').match(/<img\s+[^>]*src=["']/gi) || []).length;
  
  const trackingPixelRegex =
    /<img[^>]*(width="1"|height="1"|style="[^"]*(display:\s*none|opacity:\s*0)[^"]*")/i;

  const domains = new Set(
    [...(allText.matchAll(/https?:\/\/([^\s"'<>]+)/g) || [])].map(([, domain]) =>
      domain.replace(/\/$/, ''),
    ),
  );
  ...
  console.log({
    ...
    spfResult: email.headers?.['authentication-results']?.match(/spf=([a-z]+)/i)?.[1],
    dkimResult: email.headers?.['authentication-results']?.match(/dkim=([a-z]+)/i)?.[1],
    dmarcResult: email.headers?.['authentication-results']?.match(/dmarc=([a-z]+)/i)?.[1],
    ipSender: extractSenderIpFromReceivedHeader(email.headers?.['received']),
...
function extractSenderIpFromReceivedHeader(receivedHeader: any): string | undefined {

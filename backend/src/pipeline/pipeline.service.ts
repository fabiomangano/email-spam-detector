...
import { ResultService, SpamAnalysisResult } from '../modules/result/result.service';
...
export class PipelineService {
  ...
  async runFullPipeline(filename: string): Promise<SpamAnalysisResult> {
    ...
    try {
      ...
      const technicalResult = await this.technicalService.analyzeTechnical(parsedData);
      
      // Step 3: Behavior Analysis
      console.log('Step 3: Running behavior analysis...');
      const behaviorResult = await this.behaviorService.analyzeBehavior(parsedData);
